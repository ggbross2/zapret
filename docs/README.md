...
Один из самых популярных GUI лаунчеров для программы Запрет. Свыше 50 стратегий обхода блокировок для Discord и YouTube. Также программа предоставляет широкие возможности по настройке для автоматического запуска как гуи, так и отдельных бат стратегий. А также тонких настроек поведения любой части программы (например быстрое управление списками доменов прямо из GUI программы для всех стратегий).
